...
const HorizonHero = () => {
  ...
  const smoothCameraPos = useRef<{ x: number; y: number; z: number }>({
    x: 0,
    y: 30,
    z: 100,
  });
  const cameraVelocity = useRef<{ x: number; y: number; z: number }>({
    x: 0,
    y: 0,
    z: 0,
  });
  ...
  type ThreeRefs = {
    ...
    stars: Array<THREE.Points>;
    nebula: THREE.Mesh | null;
    mountains: Array<THREE.Mesh>;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (
        refs.camera &&
        typeof refs.targetCameraX === "number" &&
        typeof refs.targetCameraY === "number" &&
        typeof refs.targetCameraZ === "number"
      ) {
  ...
  useEffect(() => {
    ...
    // Set initial states to prevent flash, only if refs are not null
    const elements = [
      menuRef.current,
      titleRef.current,
      subtitleRef.current,
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      refs.mountains.forEach((mountain, i) => {
        ...
        if (!refs.nebula || !refs.locations || !refs.locations[i]) return;
  ...
  return (
    <div
      ref={containerRef}
      className="relative min-h-screen flex flex-col justify-between items-center overflow-x-hidden bg-black"
    >
      {/* Three.js Canvas */}
      <canvas
        ref={canvasRef}
      ...
      {/* Main content */}
      <div className="hero-content cosmos-content flex flex-col items-center justify-center min-h-screen z-10">
        <h1
          ref={titleRef}
          className="hero-title font-orbitron text-yellow-400 text-[6rem] md:text-[8rem] lg:text-[10rem] font-extrabold mb-4 tracking-tight drop-shadow-[0_0_40px_rgba(255,255,0,0.7)] transition-all duration-300 text-center hover:drop-shadow-[0_0_80px_rgba(255,255,0,1)]"
        >
          HORIZON
        </h1>
        ...
        <div
          ref={subtitleRef}
          className="hero-subtitle cosmos-subtitle text-lg md:text-2xl text-white opacity-80 text-center"
        >
          <p className="subtitle-line mb-2">Where vision meets reality,</p>
      ...
      <div className="scroll-sections relative z-10">
        {[...Array(2)].map((_, i) => {
          const titles: string[] = ["HORIZON", "COSMOS", "INFINITY"];
          const subtitles: { line1: string; line2: string }[] = [
            {
          ...
          return (
            <section
              key={i}
              className="content-section min-h-screen flex flex-col items-center justify-center"
            >
              <h1 className="hero-title text-yellow-400 text-[6rem] md:text-[8rem] lg:text-[10rem] font-extrabold mb-4 tracking-tight drop-shadow-[0_0_40px_rgba(255,255,0,0.7)] transition-all duration-300 text-center hover:drop-shadow-[0_0_80px_rgba(255,255,0,1)]">
                <span className="font-orbitron">
                  {titles[i + 1] || "DEFAULT"}
                </span>
              </h1>

              <div className="hero-subtitle cosmos-subtitle text-lg md:text-2xl text-white opacity-80 text-center">
                <p className="subtitle-line mb-2">{subtitles[i + 1]?.line1}</p>
